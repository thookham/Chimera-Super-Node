...
### Security (CISA Aligned)
- **Policy**: Added `SECURITY.md` detailing memory safety and reporting procedures.
- **Robustness**: Implemented strict logging capture for Tor, I2P, and Lokinet sidecars (using `Stdio::piped`).
- **Safety**: Hardened I2P and Lokinet config to ensure data directories exist before startup.

### Fixed
- **Repository**: Restored missing git submodules (`i2pd`, `tor`, `lokinet`, `nym`) ensuring a complete buildable tree.
- **Config**: fixed `chimera.toml` defaults to be cross-platform (Unix/Windows binary paths).
- **Code**: Fixed syntax error in `i2p.rs` adapter.

### Documentation
- **Architecture**: Added `docs/ARCHITECTURE.md` with system diagrams.
- **README**: Overhauled for clarity, best practices, and detailed installation steps.
- **Roadmap**: Updated with completed audit milestones.

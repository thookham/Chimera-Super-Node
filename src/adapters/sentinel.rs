use super::ProtocolAdapter;
use async_trait::async_trait;
use log::info;
use std::process::{Child, Command, Stdio};
use std::sync::Arc;
use tokio::sync::Mutex;

/// Sentinel Decentralized VPN Adapter
/// Cosmos-based decentralized VPN
pub struct SentinelAdapter {
    enabled: bool,
    binary_path: String,
    socks_port: u16,
    process: Arc<Mutex<Option<Child>>>,
}

impl SentinelAdapter {
    pub fn new(enabled: bool, binary_path: String, socks_port: u16) -> Self {
        Self {
            enabled,
            binary_path,
            socks_port,
            process: Arc::new(Mutex::new(None)),
        }
    }
}

#[async_trait]
impl ProtocolAdapter for SentinelAdapter {
    async fn start(&self) -> anyhow::Result<()> {
        if !self.enabled {
            info!("Sentinel dVPN is disabled, skipping start.");
            return Ok(());
        }

        info!("Starting Sentinel dVPN...");
        let child = Command::new(&self.binary_path)
            .arg("connect")
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()?;

        let mut proc_lock = self.process.lock().await;
        *proc_lock = Some(child);

        info!("Sentinel dVPN started on SOCKS port {}.", self.socks_port);
        Ok(())
    }

    async fn stop(&self) -> anyhow::Result<()> {
        let mut proc_lock = self.process.lock().await;
        if let Some(ref mut child) = *proc_lock {
            child.kill()?;
            *proc_lock = None;
            info!("Sentinel stopped.");
        }
        Ok(())
    }

    async fn is_healthy(&self) -> bool {
        let proc_lock = self.process.lock().await;
        proc_lock.is_some()
    }

    fn get_proxy_addr(&self) -> String {
        format!("127.0.0.1:{}", self.socks_port)
    }
}

...
impl Socks5Server {
    pub fn new(port: u16, tor_port: u16, i2p_port: u16) -> Self {
        Self {
            port,
    ...
    pub async fn run(&self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_connection(socket, tor, i2p).await {
...
async fn handle_connection(mut client: TcpStream, tor: SocketAddr, i2p: SocketAddr) -> anyhow::Result<()> {
    // 1. Handshake
    let mut buf = [0u8; 2];
    ...
    let (target_host, target_port) = match addr_type {
        ...
        0x03 => { // Domain
            ...
            let mut port = [0u8; 2];
            client.read_exact(&mut port).await?;
            (String::from_utf8_lossy(&domain).to_string(), u16::from_be_bytes(port))
        },
    ...
    match up_addr_type {
        0x01 => { let mut buf = [0u8; 6]; upstream.read_exact(&mut buf).await?; }
        0x03 => { 
